...
# 메소드
#
# requests라는 모듈을 불러와서 requests.get() 함수를 가지고 GET 리퀘스트를 보내는 코드입니다.
# 이런 식으로 리퀘스트 메소드에 따라서 GET 메소드면 requests.get(), POST 메소드면 requests.post() 함수를 쓰면 됩니다.
#
# 리스폰스
#
# requests 모듈에서는 리퀘스트를 보내면 리스폰스에 해당하는 값을 리턴하는데요.
# 이걸 res 라는 변수에 저장했습니다.
#
# try-except문
#
# 그러고 나서 특이한 문법이 있죠.
# 이건 try-except문이라는 건데요.
# 이 문법은 실패할 가능성이 있는 코드를 try: 블록에서 실행하고 만약에 예외적인 상황이 발생하면 except: 블록에 있는 코드를 실행해 줍니다.
#
# 네트워크 리퀘스트는 항상 100% 성공할 수는 없잖아요?
# 그래서 이런 식으로 try: 블록 안에다가 실행할 코드를 써 주고, except Except as e: 블록에서는 예외적인 상황이 발생했을 때 print(e)로 출력해 주는 거죠.
# 리퀘스트를 보내는 코드에선 되도록이면 이렇게 try-except문을 사용하는 걸 권장드려요.
#
# JSON 사용하기
#
