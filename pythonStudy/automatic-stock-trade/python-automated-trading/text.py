...
# 데드 크로스일 때 전량 매도합니다.
# 파일 구조
# PyCharm에서 프로젝트를 생성하고 진행합니다. 필요하다면 자유롭게 파일을 더 만들어도 되지만, 최소한 백테스트를 하는 파일인 backtest.py와 백테스트에 쓰는 데이터인 sample.json 그리고 실제 자동 매매를 구현한 main.py 파일을 만들어 주세요. 참고로 .venv/ 폴더는 PyCharm에서 자동 생성해 주는 폴더입니다.
#
#
...
#
# 한국투자증권 당일분봉조회 API에서 가져온 데이터들을 JSON 형식으로 저장한 것입니다. output2에 해당하는 값인데요. 데이터 값의 형식은 아래 표를 참고해 주세요.
#
# 속성명	설명
